...
import { getAllVactions } from "../../../../helper/fetchers/Vacations.jsx";
import { toast } from "react-toastify";
import moment from "moment";
import Progress from "../../../../Components/Progress.jsx";
import { getAllVacationCategories } from "../../../../helper/fetchers/VacationCategory.jsx";
...
const HolidayMangment = () => {
  ...
  const [vacations, setVactions] = useState(null);
  ...
  const getVactions = async () => {
    try {
      const { data } = await getAllVactions();
      console.log(data);

      if (data?.success) {
        setVactions(data?.allVacations);
      } else {
  ...
  const [vactionCategory, setVactionCategory] = useState(null);
  const getvactionCategoryId = async () => {
    try {
      ...
      if (data?.vacations) {
        ...
        // });
        setVactionCategory(data.vacations);
      } else {
        console.log("Data retrieval failed");
      }
    } catch (error) {
      ...
      <div className="grid grid-cols-12 gap-2">
        <div className="col-span-3">
          <div className={`${style.holidayTypesContainer} py-5 !px-0`}>
            ...
            {vactionCategory && vactionCategory.length > 0 ? (
              vactionCategory.map(({ name, filesVaction }) => (
          ...
          <div className={`${style.allhoildays} allhoildaysContainer p-4`}>
            ...
            <fieldset className={`${style.allhoildaysTables} mt-5`}>
              ...
              {/* <DataTableComponent
                ...
                columns={columns}
              /> */}
              <div className="mt-3 !h-auto !overflow-auto scrollbar-none">
                {vacations ? (
                  <CustomTable
                      ...
                      ? vacations.map(
                          (
                            ...
                            >
                              ...
                              <TableCell>{employeeId?.firstName}</TableCell>
