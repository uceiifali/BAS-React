...
const Expenses = () => {
  ...
  const [expensesCleanderValue, setExpensesCleanderValue] = useState(
    new Date()
  );
    ...
    <div>
      {chooseDateItems && (
        ...
        >
          <Modal.Body className="d-flex align-items-center">
            <Calendar
              onChange={handleCleanderItemsValue}
              className={"bg-[#1E1E2D]"}
              value={ItemscleanderValue}
            />
          </Modal.Body>
        </Modal>
      )}
      ...
      {chooseExpensesDate && (
        ...
        >
          <Modal.Body className="d-flex align-items-center">
            <Calendar
              onChange={handleCleanderExpenssValue}
              className={"bg-[#1E1E2D]"}
              value={expensesCleanderValue}
            />
          </Modal.Body>
        </Modal>
      )}
      ...
      <fieldset
        className={`${style.ExpensesContainer} mt-5  overflow-x-hidden `}
      >
        <legend className="text-center ">كل المصروفات</legend>
        <div
          className={`bg-[#0A0D10] my-4   p-4 ${style.allexpensesConatiner}`}
        >
          <svg
            onClick={() => {
              setChooseExpensesDate(true);
            }}
            className="pointer"
            xmlns="http://www.w3.org/2000/svg"
            width="18"
