...
const ShowClientDetails = ({ showProject, setShowProject }) => {
  ...
  const ClientDetailsTable = Array.from({ length: 2 }).map((_, index) => {
    return {
      ...
      status: <div className='mt-2 d-flex flex-column justify-content-center align-iems-center gap-1 my-1'>
        <svg className='mx-auto' xmlns="http://www.w3.org/2000/svg" width="19" height="19" viewBox="0 0 19 19" fill="none">
            ...
            <div className='d-flex justify-content-between  w-90 mx-auto'>
              <div className='d-flex gap-2 w-100'>
                <div
                  ...
                  className={`d-flex justify-content-center align-items-center p-2 w-25 pointer   my-3 ${clientStatusType === "معلومات المشروع" ? "edit-clint-header-golden" : "edit-clint-header-white"} `}>
                  <p className={`
                ${clientStatusType === "معلومات المشروع" ? "golden" : "text-white"} my-auto text-center`}> معلومات المشروع </p>
                </div>
                <div
                  onClick={() => {
                    setClientStatusType("الدفعات")
                  }}
                  className={` w-25 pointer d-flex justify-content-center p-2 align-items-center  my-3 ${clientStatusType === "الدفعات" ? "edit-clint-header-golden" : "edit-clint-header-white"} `}>
                  <p className={` text-center my-auto ${clientStatusType === "الدفعات" ? "golden" : "text-white"}
